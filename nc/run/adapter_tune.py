...
#from src.transformers.utils import get_full_repo_name
#from src.transformers.utils import get_full_repo_name
...
from ...models.roberta_adapter import RobertaForSequenceClassification_adapter
...
def main():
    ...
    for k, v in vars(args).items():
        if ('attn' in k or 'ffn' in k) and not hasattr(config, k):
            setattr(config, k, v)
    config.finetune_layers=list(range(args.last_finetune_layer- args.num_finetune_layers+1, args.last_finetune_layer+1))
    ...
    #logger.info(config.num_hidden_layers)
    #logger.info(config.chosen_token)
    logger.info(config)
    model = RobertaForSequenceClassification_adapter.from_pretrained(
    ...
    for i in range(args.last_finetune_layer- args.num_finetune_layers+1, args.last_finetune_layer+1):
        ...
        for n,p in model.roberta.encoder.layer[i].named_parameters():
            if 'ef_' in n or 'LayerNorm' in n:
                p.requires_grad = True
    
    # todo compute trainable params
        ...
        with open(os.path.join(args.output_dir, 'all_results.json'), 'r') as f:
            try:
                ...
                if 'best_accuracy' in result:
                    best_metric = float(result['best_accuracy'])
                else:
                    best_metric = float(result['eval_accuracy'])
            except:
