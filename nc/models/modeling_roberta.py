...
from transformers.models.activations import ACT2FN, gelu
...
class RobertaSelfAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        ...
        #new
        if self.attn_mode == 'adapter' and self.config.attn_option == 'parallel':
...
class RobertaLayer(nn.Module):
    def __init__(self, config, add_adapter=False):
        super().__init__()
        # new
        self.config = copy.deepcopy(config)
        if add_adapter==False:
            '''
            if config.ffn_mode!="none":
                config.ffn_mode_tmp=config.ffn_mode
                config.attn_mode_tmp=config.attn_mode
            '''
            self.config.ffn_mode="none"
            self.config.attn_mode="none"
        '''
        else:
            config.ffn_mode=config.ffn_mode_tmp
            config.attn_mode=config.attn_mode_tmp
        '''

        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = RobertaAttention(self.config)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            if not self.is_decoder:
                raise ValueError(f"{self} should be used as a decoder model if cross attention is added")
            self.crossattention = RobertaAttention(self.config, position_embedding_type="absolute")
        self.intermediate = RobertaIntermediate(self.config)
        self.output = RobertaOutput(self.config)
...
class RobertaEncoder(nn.Module):
    def __init__(self, config):
        ...
        if hasattr(config, 'finetune_layers'):
            for i in config.finetune_layers:
                self.layer[i] = RobertaLayer(config, add_adapter=True)
                #self.layer.i = RobertaLayer(config, add_adapter=True)
...
class RobertaForSequenceClassification(RobertaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        #self.classifier = LinearClassificationHead(config)
        print('initializing model from here')
...
class LinearClassificationHead(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.token = config.chosen_token
    ...
    def forward(self, features, **kwargs):
        if self.token == 'BOS':
            ...
            x = torch.mean(features[:, 1:-1, :],1)  # take <s> token (equiv. to [CLS])
        #x = features[:, 0, :]  # take <s> token (equiv. to [CLS])
        '''
        x = self.dropout(x)
        x = self.dense(x)
